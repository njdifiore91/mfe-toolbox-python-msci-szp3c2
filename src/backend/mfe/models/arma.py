"""
MFE Toolbox - ARMA Module

This module implements AutoRegressive Moving Average (ARMA) time series models
with robust parameter estimation, diagnostic tools, and forecasting capabilities.
It is optimized with Numba for performance-critical calculations.
"""
...
from ..utils.validation import validate_array, ensure_stationary, ensure_invertible
from ..core.optimization import OptimizationResult, minimize
from ..utils.async_helpers import AsyncTaskManager
from ..core.distributions import normal_loglikelihood
from ..core.testing import adf_test
from ..utils.numpy_helpers import convert_to_numpy_array
from ..utils.pandas_helpers import to_pandas_dataframe

# Module constants
ARMA_MAX_LAG = 100  # Maximum lag order for ARMA models
...
def compute_arma_residuals(
    ...
    """
    Computes residuals from an ARMA model with given parameters and data.
...
@jit_decorator(nopython=True)
def compute_arma_loglikelihood(
    params: np.ndarray,
    y: np.ndarray,
    p: int,
    q: int,
    ...
    """
    Computes the log-likelihood of an ARMA model for given parameters.
    ...
    # Compute residuals
    residuals = compute_arma_residuals(y, ar_params, ma_params, constant, exog, exog_params)
...
def arma_forecast(
    ...
    """
    Generates forecasts from an ARMA model with given parameters.
...
def compute_forecast_variance(
    ...
    """
    Computes the variance of forecasts from an ARMA model.
    ...
    -----
    This function is optimized with Numba for performance.
    It computes forecast variances using the MA-infinity representation.
    """
    # Get dimensions
    p = len(ar_params)
    q = len(ma_params)
    ...
    # Initialize psi weights (MA-infinity representation)
    # We need to compute enough psi weights for the forecast horizon
    psi = np.zeros(max(q, steps) + 1)
    psi[0] = 1.0  # First coefficient is always 1
    ...
    for i in range(1, len(psi)):
        ...
        # AR component contribution (recursive)
        for j in range(1, min(i, p+1)):
            psi[i] += ar_params[j-1] * psi[i-j]
    
    # Compute forecast variances
    for h in range(steps):
        # Sum squared psi weights from h onwards for h-step-ahead forecast variance
        for j in range(h+1):
            forecast_vars[h] += psi[j]**2
        
        # Scale by residual variance
        forecast_vars[h] *= residual_variance
...
def arma_order_select(
    ...
    """
    Selects optimal ARMA order based on information criteria.
    ...
    if max_ar > ARMA_MAX_LAG or max_ma > ARMA_MAX_LAG:
        raise ValueError(f"Maximum AR and MA orders must not exceed {ARMA_MAX_LAG}")
    ...
    for p in range(max_ar + 1):
        for q in range(max_ma + 1):
            ...
            try:
                ...
                model = ARMA(p=p, q=q, include_constant=include_constant)
                ...
                logger.debug(f"Error estimating ARMA({p}, {q}): {str(e)}")
                continue
    
    logger.info(f"Best ARMA order selected: ({best_p}, {best_q}) with {ic}={best_ic:.4f}")
...
class ARMA:
    """
    AutoRegressive Moving Average time series model with parameter estimation,
    ...
    def __post_init__(self):
        ...
        if self.p > ARMA_MAX_LAG or self.q > ARMA_MAX_LAG:
            raise ValueError(f"AR and MA orders must not exceed {ARMA_MAX_LAG}")
        ...
        # Check if model is non-trivial
        if self.p == 0 and self.q == 0 and not self.include_constant:
            logger.warning("Model has no parameters. At least one of p, q, or include_constant should be positive/True.")
    ...
    def estimate(
        ...
        if n_params == 0:
            raise ValueError("Model has no parameters. At least one of p, q, or include_constant should be positive/True.")
        ...
        def objective(params):
            return compute_arma_loglikelihood(params, y, self.p, self.q, 
        ...
        try:
            ...
            if self.p > 0:
                is_stationary = ensure_stationary(self.ar_params, raise_error=False)
                if not is_stationary:
                    logger.warning("Estimated AR parameters do not satisfy stationarity condition")
            
            if self.q > 0:
                is_invertible = ensure_invertible(self.ma_params, raise_error=False)
                if not is_invertible:
                    logger.warning("Estimated MA parameters do not satisfy invertibility condition")
            
            # Compute and store residuals
            self.residuals = compute_arma_residuals(
            ...
            logger.error(f"Error estimating ARMA model: {str(e)}")
            raise
    ...
    async def estimate_async(
        ...
        """
        Asynchronously estimates ARMA model parameters.
        ...
        # Create task manager for async execution
        task_manager = AsyncTaskManager()
        
        # Create and run optimization task asynchronously
        try:
            # Run the optimization in a separate thread to avoid blocking
            optimization_task = task_manager.create_task(
                asyncio.to_thread(
                    minimize,
                    objective, 
                    initial_params,
            ...
            # Compute and store residuals (in a separate thread to avoid blocking)
            compute_residuals_task = task_manager.create_task(
                asyncio.to_thread(
            ...
            self.residuals = await compute_residuals_task
            ...
            logger.error(f"Error estimating ARMA model asynchronously: {str(e)}")
            raise
    ...
    def forecast(
        ...
        """
        Generates forecasts from estimated ARMA model.
    ...
    async def forecast_async(
        ...
        """
        Asynchronously generates forecasts from estimated ARMA model.
        ...
        forecast_task = task_manager.create_task(
            asyncio.to_thread(
                arma_forecast,
                y, self.ar_params, self.ma_params, self.constant, steps,
                self.residuals, None, exog_forecast, self.exog_params
    ...
    def forecast_variance(self, steps: int = 1) -> np.ndarray:
        ...
        # Compute forecast variances
        variances = compute_forecast_variance(self.ar_params, self.ma_params, self.sigma2, steps)
    ...
    def simulate(
        ...
        """
        Simulates time series from ARMA model with given parameters.
    ...
    def diagnostic_tests(self, tests: List[str] = ['ljung_box', 'jarque_bera']) -> Dict[str, Any]:
        ...
        for test in tests:
            if test.lower() == 'ljung_box':
                ...
                adf_stat, adf_pval = adf_test(self.residuals)
                results['adf'] = {
                    'statistic': adf_stat,
                    'p_value': adf_pval
                }
    ...
    def information_criteria(self, criteria: List[str] = ['aic', 'bic', 'hqic']) -> Dict[str, float]:
        ...
        # If a single criterion was requested, return its value directly
        if len(results) == 1 and isinstance(criteria, str):
    ...
    def is_stationary(self) -> bool:
        """
        Checks if AR parameters satisfy stationarity condition.
        
        Returns
        -------
        bool
            True if AR process is stationary
        """
        # Check if model has been estimated
        if self.ar_params is None:
            raise ValueError("Model must be estimated before checking stationarity")
        
        # If no AR component, return True
        if len(self.ar_params) == 0:
            return True
        
        # Check stationarity
        return ensure_stationary(self.ar_params, raise_error=False)
    
    def is_invertible(self) -> bool:
        """
        Checks if MA parameters satisfy invertibility condition.
        
        Returns
        -------
        bool
            True if MA process is invertible
        """
        # Check if model has been estimated
        if self.ma_params is None:
            raise ValueError("Model must be estimated before checking invertibility")
        
        # If no MA component, return True
        if len(self.ma_params) == 0:
            return True
        
        # Check invertibility
        return ensure_invertible(self.ma_params, raise_error=False)
    ...
    @classmethod
    def from_dict(cls, model_dict: Dict[str, Any]) -> 'ARMA':
        """
        Creates ARMA model instance from dictionary specification.
        ...
        -------
        ARMA
            ARMA model instance with loaded parameters
        """
        # Check model type
        if model_dict.get('model') != 'ARMA':
            raise ValueError("Dictionary does not contain an ARMA model specification")
    ...
    def _create_summary(self) -> None:
        ...
        model_info = {
            'Model': f"ARMA({self.p}, {self.q}){' with constant' if self.include_constant else ''}",
